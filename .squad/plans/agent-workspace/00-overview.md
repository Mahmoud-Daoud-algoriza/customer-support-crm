...
| **14 — tasks and internal notes** | **1–7** (the two entities, their EF configurations and the `TasksAndInternalNotes` migration, the two Application services, the five `§5.6` endpoints, the seeded note and task, and the thirteen tests — including the previously skipped `InternalNotesAreUnreachableTests`), **9** (internal-notes region), **10** (tasks region). **Task 8 NOT built** | ⚠ **Implemented 2026-09-06; verification PARTIAL** — backend, data layer and live stack verified; **front-end screen checks outstanding** (no browser-driving capability). ⛔ **Task 8 blocked on S9-1** — AC 2 open pending the user's Option A / Option B decision |
