...
| **Delivery** | Row 10 — the 18 stories implemented *and* verified | 65% | **9 of 18** | 9 ÷ 18 × 65 = **32.5%** |
| | | **100%** | | **= 67.5%** |
...
**Stories 01, 02, 03, 04, 05, 06 and 07 are Implemented and Verified** — 01–03 close phase 1, **04
...
| 06 | `ticket-lifecycle` | ticket-management | **T1** | Intake Complete | **Plan Complete** | ✅ **Implemented** — all 11 tasks | ✅ **Verified** 2026-08-31 | 05 | — · **OQ-3 closed 2026-08-31 (A-21)**, and **A-21 is applied through the shared policy, not re-implemented** · delivered **whole, not sliced** · **completes story 04's interaction-timeline criterion** · **finding I-23**: A-16's customer column reaches a customer only when story 13 publishes the portal route · **F-1 stays open by design** |
...
### 6.8 Implementation findings — story 04, the I-9 / I-10 fixes, story 05, the A-21 foundation, story 06 and story 07 (2026-08-27 / 31)
...
| **I-23** | **The plan's tests 5–8, 10 and 13 cannot run as written in this story.** Each is a *customer* calling a transition endpoint, but no endpoint exists that a `Customer` may reach: the staff controller is `RequireAgent` by class policy (task 7's own instruction), and the portal route `POST /portal/tickets/{id}/transition` is published by **Story 13**, which calls this same service. Adding it here would be starting Story 13 | **A-16's customer column is asserted against `TransitionAuthority` directly — the exact predicate the portal endpoint will call — plus the `403` a customer actually receives today over HTTP.** `The_customer_column_of_A16_has_exactly_two_permitted_cells` walks the full 6×6 matrix and asserts the permitted set is exactly *cancel-own-while-`New`* and *reopen-own-`Resolved`*, which is stronger than the six examples the plan lists, and `A_customer_role_token_is_refused_by_the_role_gate_on_every_lifecycle_route` covers all three new routes. **Nothing about A-16 is unproven; only its delivery to a customer is deferred**, and it is deferred by the plan's own task 7, not by this story's choice. The client half is pinned too, in `transition-matrix.spec.ts` |
...
**Stories 01, 02, 03, 04, 05, 06, 07 and 12 are implemented and verified, story 16 in full with
them.**
...
| Working tree | 🟡 **Story 07 uncommitted, on `cd1629b`** | Re-read from the repository on **2026-08-31**, not carried over: `main` head is **`cd1629b`**, *"feat: story 06 — ticket lifecycle"*, and `git status --short` lists **39 paths — story 07's whole change set, deliberately uncommitted**, because CLAUDE.md §3 commits only when asked. Every one of the 39 is a path story 07's plan predicts: the three Domain files, the EF configuration and its migration, the four Application files, the two controllers, the seeder, the two test files and the two test helpers they needed, and on the front end the two clients, the two shared components, the thread region, the two portal stubs, the routes, the stylesheet and both dictionaries. **No unexpected path.** *(This row previously read "Clean at `ca2b306`", dated 2026-08-30 — it had been stale since story 05 landed, and is corrected here rather than carried forward.)* The chain stays linear: `54abd75` (story 16 Part A + slice 1) → `a4230b9` (slice 2) → `5c716b1` (slice 3) → `60b58e6` (front-end error layer) → `346d28b` (slice 4) → `cf22841` (the I-9 / I-10 fixes) → `c0c0043` (slice 5) → `6e431dd` (this manual, the workflow skills, and the change-log extraction) → `ca2b306` (slice 6) → `d3f3df5` → `6812e91` (A-20) → `a633923` (story 05) → `e3a111c` → `b13b7d8` (A-21) → `cd1629b` (story 06). **Nothing has been rewritten, reordered or squashed.** `6e431dd` is where §9's 900 lines went — **moved to [CHANGELOG-IMPLEMENTATION.md](CHANGELOG-IMPLEMENTATION.md), not deleted**, which the file diff confirms line for line. **`git status` is the live source — this row is a snapshot, not a claim to be trusted over the repository** |
