...
| **Current focus** | Story 04 **slice 5 of 6** complete and verified — `POST /auth/register` and A-15's three outcomes (plan task 7), which closes **S9-7**. Registration links to an agent-created profile rather than duplicating it, and **finding I-5 is resolved**: the configured default branch is the *customer's*, and the portal login carries none. Two cross-cutting contract fixes landed before it — **AP-10** is now enforced (I-9) and every model-state `400` carries the AP-2 slug and §6.12 envelope (I-10). **Awaiting explicit approval before slice 6** (tasks 11–14, the front-end screens). Story 04 is being delivered in slices at the user's instruction; the slice map is in [00-overview.md](../.squad/plans/customer-management/00-overview.md). Phase order per [00-implementation-plan.md](../.squad/plans/00-implementation-plan.md) §3 |
...
**Next:** story 04 **slice 6** (the front-end screens, tasks 11–14), then
...
0. **Story 04 slice 4 is done and verified — awaiting explicit approval before slice 5.** Story 04
   ...
   | 6 — front end | 11, 12, 13, 14 | ⬜ Next, blocked on approval |
   ...
   **Eleven findings have come out of these four slices and the two contract fixes, and none is
   ...
   contract publishes is affected, and its only visible consequence is two leftover dev rows. **I-5 is
   closed by slice 5:** A-15's "configured default branch" sets `Customer.branchId` only —
   `User.branchId` stays null, which four approved documents agree on and a test now pins. The
