...
| **Delivery** | Row 10 — the 18 stories implemented *and* verified | 65% | **6 of 18** | 6 ÷ 18 × 65 = **21.7%** |
| | | **100%** | | **= 56.7%** |

**A story counts only when it is implemented *and* verified in full.** Story 06 now does: all
**eleven** plan tasks are implemented and verified, so the delivery track moves from 5 to 6 and the
total from 53.1% to **56.7%**. Like story 05 it was delivered **whole rather than in slices**, so the
counting convention that made story 04 sit at zero through five slices did not bite here either.

**Six of eighteen is a third of the delivery track, and it is the third that matters most.** Stories
05 and 06 are the two the assessment's core loop rests on, and both are **T1 and uncuttable**. What
remains is broader but shallower: the portal, the dashboards, the KB, the AI seam and the adapters.

**Stories 04 and 05 are not re-litigated here.** Story 04 counted from 2026-08-30 (fourteen plan
tasks, six slices verified) and story 05 from 2026-08-31 (thirteen plan tasks).
...
should hold story 06 open, the delivery count returns to 5 of 18 and the total to 53.1%** — and the
...
**Stories 01, 02, 03, 04, 05 and 06 are Implemented and Verified** — 01–03 close phase 1, **04 is
the first T1 business story delivered end to end**, and **05 and 06 close phase 3 with the ticket
core and its lifecycle, the loop the whole assessment rests on**. **Stories 07–18 are at
...
### 6.8 Implementation findings — story 04, the I-9 / I-10 fixes, story 05, the A-21 foundation, and story 06 (2026-08-27 / 31)
...
**Next:** **story 07** (`ticket-intake-messaging`) — **blocked on explicit approval only**, and no
open question gates it. It supplies the **trigger** for the automatic `Pending → Open` rule story 06
implemented (R-13, R-14): `ApplyAutomaticCustomerReplyTransitionAsync` exists and is tested, and
story 07's portal message endpoint is its only caller. Story 16 **Part B** is **now unblocked** —
story 06 writes the `TicketStatusChanged` and `TicketEscalated` audit rows its log needs — but stays
at Phase 7 by execution order.
...
0. **Story 06 is complete — implemented and verified 2026-08-31. Phase 3 and the assessment's core
